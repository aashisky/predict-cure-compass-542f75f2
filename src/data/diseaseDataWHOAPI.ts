// WHO-style disease surveillance data — fetches live data from WHO GHO & DON APIs with static fallbacks
...
const WHO_DON_API = 'https://www.who.int/api/hk/news/disease-outbreak-news';

interface WHOGHORecord {
  Id: number;
...
interface WHODONResponse {
  value: WHODONItem[];
}

/** Maps disease IDs to WHO GHO indicator codes for live data fetching */
const WHO_INDICATOR_MAP: Record<string, { cases?: string; deaths?: string }> = {
  cholera:      { cases: 'CHOLERA_0000000001', deaths: 'CHOLERA_0000000002' },
  tuberculosis: { cases: 'TB_e_inc_num', deaths: 'TB_e_mort_exc_tbhiv_num' },
  malaria:      { cases: 'MALARIA_EST_CASES', deaths: 'MALARIA_EST_DEATHS' },
  measles:      { cases: 'WHS3_49' },
  pertussis:    { cases: 'WHS3_43' },
  diphtheria:   { cases: 'WHS3_42' },
  leprosy:      { cases: 'WHS3_51' },
  polio:        { cases: 'WHS3_57' },
  tetanus:      { cases: 'WHS3_56' },
  yellowfever:  { cases: 'YFV_REPORTED_CASES' },
  plague:       { cases: 'WHS3_55' },
};

async function fetchGHOIndicator(code: string, year: number): Promise<WHOGHOResponse> {
  const safeYear = Math.floor(Number(year));
  const url = `${WHO_GHO_BASE}/${encodeURIComponent(code)}?$filter=TimeDim+eq+'${safeYear}'`;
...
// ═══════════════════════ WHO API LIVE DATA FETCHERS ═══════════════════════

/**
 * Fetch live disease data from WHO GHO API.
 * For diseases with mapped WHO indicators, fetches the latest available annual data
 * and merges it with the static fallback dataset.
 * Unmapped diseases retain their fallback values.
 */
export async function fetchWHODiseases(): Promise<DiseaseData[]> {
  const currentYear = new Date().getFullYear();
  const result = diseases.map(d => ({ ...d }));

  const indicatorFetches: Array<{
    diseaseIdx: number;
    field: 'cases' | 'deaths';
    code: string;
  }> = [];
  ...
  for (let i = 0; i < result.length; i++) {
    const indicators = WHO_INDICATOR_MAP[result[i].id];
    if (!indicators) continue;
    if (indicators.cases) indicatorFetches.push({ diseaseIdx: i, field: 'cases', code: indicators.cases });
    if (indicators.deaths) indicatorFetches.push({ diseaseIdx: i, field: 'deaths', code: indicators.deaths });
  }

  await Promise.allSettled(
    indicatorFetches.map(async ({ diseaseIdx, field, code }) => {
      for (const yr of [currentYear, currentYear - 1, currentYear - 2]) {
        const resp = await fetchGHOIndicator(code, yr);
        const total = resp.value
          .filter(r => r.NumericValue != null)
          .reduce((sum, r) => sum + (r.NumericValue ?? 0), 0);
        if (total > 0) {
          result[diseaseIdx] = { ...result[diseaseIdx], [field]: Math.round(total) };
          return;
        }
      }
    })
  ...
  // Recalculate CFR for diseases that received live data
  const updatedIndices = new Set(indicatorFetches.map(f => f.diseaseIdx));
  for (const idx of updatedIndices) {
    const d = result[idx];
    if (d.cases > 0 && d.deaths > 0) {
      result[idx] = { ...d, cfr: Math.round((d.deaths / d.cases) * 10000) / 100 };
    }
  }
...
/**
 * Fetch disease outbreak news from WHO DON API.
 * Returns AlertItem[] mapped from WHO's disease outbreak news feed.
 * Falls back to static alerts if the API is unavailable.
 */
export async function fetchWHOAlerts(): Promise<AlertItem[]> {
  try {
    const res = await fetch(WHO_DON_API);
    if (!res.ok) throw new Error(`WHO DON API: ${res.status}`);
    const data: WHODONResponse = await res.json();

    return data.value.slice(0, 20).map((item, i) => ({
      id: item.Id || String(i + 1),
      timestamp: item.DatePublished || new Date().toISOString(),
      severity: classifyDONSeverity(item.Title),
      title: item.Title,
      description: item.Summary?.replace(/<[^>]*>/g, '') || '',
      region: extractRegionFromDON(item.Title),
    }));
  } catch {
    return [...alerts];
  }
...
/**
 * Fetch country-level breakdown from WHO GHO for a specific disease.
 * Falls back to the deterministic getTopCountries() if unavailable.
 */
export async function fetchWHOCountryData(diseaseId: string): Promise<CountryData[]> {
  const indicators = WHO_INDICATOR_MAP[diseaseId];
  const disease = diseases.find(d => d.id === diseaseId);
  if (!indicators?.cases || !disease) return disease ? getTopCountries(disease) : [];

  try {
    const currentYear = new Date().getFullYear();
    for (const yr of [currentYear, currentYear - 1, currentYear - 2]) {
      const resp = await fetchGHOIndicator(indicators.cases, yr);
      const countries = resp.value
        .filter(r => r.SpatialDimType === 'COUNTRY' && r.NumericValue != null && r.NumericValue > 0)
        .map(r => ({
          country: r.SpatialDim,
          iso: r.SpatialDim,
          cases: Math.round(r.NumericValue!),
          deaths: 0,
        }))
      ...
      if (countries.length > 0) {
        if (indicators.deaths) {
          try {
            const deathResp = await fetchGHOIndicator(indicators.deaths, yr);
            const deathMap = new Map(
              deathResp.value
                .filter(r => r.NumericValue != null)
                .map(r => [r.SpatialDim, Math.round(r.NumericValue!)] as const)
            );
            countries.forEach(c => { c.deaths = deathMap.get(c.iso) ?? 0; });
          } catch { /* deaths unavailable, keep 0 */ }
        }
        return countries;
      }
    }
  ...
  return getTopCountries(disease);
}

/**
 * Fetch WHO region-aggregated data.
 * Currently returns fallback data — extend with GHO regional queries as needed.
 */
export async function fetchWHORegions(): Promise<RegionData[]> {
  return [...regions];
}

/**
 * Master fetch: returns all live data from WHO APIs with static fallbacks.
 * Use with React Query: useQuery({ queryKey: ['who-data'], queryFn: fetchAllWHOData })
 */
export async function fetchAllWHOData() {
  const [liveDiseases, liveAlerts] = await Promise.allSettled([
    fetchWHODiseases(),
    fetchWHOAlerts(),
  ]);

  return {
    diseases: liveDiseases.status === 'fulfilled' ? liveDiseases.value : [...diseases],
    alerts: liveAlerts.status === 'fulfilled' ? liveAlerts.value : [...alerts],
    regions: [...regions],
    genomicVariants: [...genomicVariants],
    outbreakTimeline: [...outbreakTimeline],
    correlationMatrix: [...correlationMatrix],
  };
}

// ─── Internal helpers for WHO DON classification ───

function classifyDONSeverity(title: string): 'info' | 'warning' | 'critical' {
  const t = title.toLowerCase();
  if (t.includes('outbreak') || t.includes('emergency') || t.includes('death') || t.includes('fatal')) return 'critical';
  if (t.includes('surge') || t.includes('increase') || t.includes('alert') || t.includes('risk')) return 'warning';
  return 'info';
}

function extractRegionFromDON(title: string): string {
  const regionKeywords: Record<string, string[]> = {
    'Africa': ['africa', 'nigeria', 'congo', 'kenya', 'ethiopia', 'south africa', 'ghana', 'cameroon', 'mozambique', 'uganda', 'tanzania', 'rwanda', 'madagascar'],
    'Americas': ['america', 'brazil', 'united states', 'mexico', 'colombia', 'argentina', 'haiti', 'peru'],
    'South-East Asia': ['india', 'indonesia', 'bangladesh', 'thailand', 'nepal', 'myanmar', 'sri lanka'],
    'Europe': ['europe', 'germany', 'france', 'united kingdom', 'spain', 'italy', 'russia'],
    'Eastern Mediterranean': ['pakistan', 'egypt', 'iraq', 'iran', 'afghanistan', 'saudi', 'yemen', 'jordan', 'lebanon'],
    'Western Pacific': ['china', 'japan', 'philippines', 'australia', 'vietnam', 'cambodia', 'korea'],
  };
